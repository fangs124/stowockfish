...
impl ChessBoard {
    ...
    pub const fn is_square_attacked(&self, square: usize, attacker_side: Side) -> bool {
        ...
        match attacker_side {
            Side::White => {
                return 
                    (B_PAWN_ATTACKS[square].data & self.piece_bbs[5].data) != 0u64 ||
            ...
            Side::Black => {
                 return 
                    (W_PAWN_ATTACKS[square].data & self.piece_bbs[11].data) != 0u64 ||
        ...
        if let Some(piece) = self.mailbox[square] {
            ...
            match piece {
                ...
                (Side::White,_) => {
                    ...
                    return new_chessboard.king_is_in_check(Side::Black);
                    /*
                ...
                (Side::Black,_) => {
                    ...
                    if let Some(king_pos) = self.piece_bbs[6].lsb_index() {
                        ...
                        new_chessboard.mailbox[cpt_index(piece)] = None;
                        //todo: more efficient check test?
                        if new_chessboard.king_is_in_check(Side::Black) {return true};
        ...
        match king_side {
            Side::White => {
                ...
                if let Some(square) = self.piece_bbs[0].lsb_index() {
                    return self.is_square_attacked(square,Side::Black);
                }
                else {
            ...
            Side::Black => {
                assert!(self.piece_bbs[6].data.count_ones() == 1);
                    if let Some(square) = self.piece_bbs[6].lsb_index() {
                        return self.is_square_attacked(square,Side::White);
                    }
                    else {
                        panic!("king_is_in_check error: bitboard is zero!")
                    }
                        ...
                        01 => { // white queens: Q
                            while bitboard.data != 0 {
                                if let Some(source) = bitboard.lsb_index() {
                                    ...
                                    //pinned piece case
                                    if self.piece_is_pinned(source) { //knights can't unpin themselves
                                        is_pinned = true;
                                        //bitboard = bitboard.pop_bit(source); continue
                                    };
                                    let data: u64 = get_queen_attack(source, blockers).data & !w_blockers.data;
                                    let mut attacks = BB {data};
                                    while attacks.data != 0 {
                                        if let Some(target) = attacks.lsb_index() {
                                            // if piece is pinned, only consider moves along pinning-rays
                                            if is_pinned {
                                                assert!(self.piece_bbs[0].data.count_ones() == 1);
                                                let king_pos = match self.piece_bbs[0].lsb_index() {
                                                    Some(x) => x,
                                                    None => panic!("generate_moves error: king not found!"),
                                                };
                                                let data = blockers.data & !(1u64 << source);
                                                let ray_bb = BB{data};
                                                let mut pin_mask = BB{data};
                                                // enemy queen
                                                let mut enemy_queens = self.piece_bbs[7];
                                                while enemy_queens.data != 0 {
                                                    let i = match enemy_queens.lsb_index() {
                                                        Some(x) => x,
                                                        None => unreachable!(),
                                                    };
                                                    let attack_ray = get_queen_attack(king_pos, ray_bb);
                                                    
                                                    // if enemy piece is in attack ray, 
                                                    if attack_ray.data & (1u64 << i) != 0 {
                                                        let ray_data = get_queen_attack(i,ray_bb).data | (1u64<<i);
                                                        pin_mask.data |= ray_data & attack_ray.data;
                                                    }
                                                    enemy_queens = enemy_queens.pop_bit(i);
                                                }
                                                // enemy bishop
                                                let mut enemy_bishops = self.piece_bbs[9];
                                                while enemy_bishops.data != 0 {
                                                    let i = match enemy_bishops.lsb_index() {
                                                        Some(x) => x,
                                                        None => unreachable!(),
                                                    };
                                                    let attack_ray = get_bishop_attack(king_pos, ray_bb);
                                                    
                                                    // if enemy piece is in attack ray, 
                                                    if attack_ray.data & (1u64 << i) != 0 {
                                                        let ray_data = get_bishop_attack(i,ray_bb).data | (1u64<<i);
                                                        pin_mask.data |= ray_data & attack_ray.data;
                                                    }
                                                    enemy_bishops = enemy_bishops.pop_bit(i);
                                                }
                                                // enemy rook
                                                let mut enemy_rooks = self.piece_bbs[10];
                                                while enemy_rooks.data != 0 {
                                                    let i = match enemy_rooks.lsb_index() {
                                                        Some(x) => x,
                                                        None => unreachable!(),
                                                    };
                                                    let attack_ray = get_rook_attack(king_pos, ray_bb);
                                                    
                                                    // if enemy piece is in attack ray, 
                                                    if attack_ray.data & (1u64 << i) != 0 {
                                                        let ray_data = get_rook_attack(i,ray_bb).data | (1u64<<i);
                                                        pin_mask.data |= ray_data & attack_ray.data;
                                                    }
                                                    enemy_rooks = enemy_rooks.pop_bit(i);
                                                }
                                                //only consider moves along pinning axis
                                                if pin_mask.data & (1u64 << target) == 0 {
                                                    let new_attacks = attacks.pop_bit(target);
                                                    assert!(attacks.data.count_ones() - new_attacks.data.count_ones() == 1);
                                                    attacks = new_attacks;
                                                    continue;
                        ...
                        05 => { // white pawns: P
                            while bitboard.data != 0 {
                                if let Some(source) = bitboard.lsb_index() {
                                    ...
                                    let mut is_lateral_pinned  = false;
                                    //pinned piece case
                                    if self.piece_is_pinned(source) {
                                        let king_pos = match self.piece_bbs[6].lsb_index() {
                                            Some(x) => x,
                                            None => panic!("generate_moves error: black king not found!"),
                                        };
                                        //check diagonal pins
                                        let new_blockers = BB{data: blockers.data & !(1u64<<source)};
                                        if (self.piece_bbs[1].data | self.piece_bbs[3].data) & get_bishop_attack(king_pos, new_blockers).data != 0 {
                                            is_diagonal_pinned = true;
                                        } 
                                        //check lateral pins
                                        if (self.piece_bbs[1].data | self.piece_bbs[4].data) & get_rook_attack(king_pos, new_blockers).data != 0 {
                                            is_lateral_pinned = true;
                                        } 
                                    }
                                    // pawn moves
                                    let target = source + 8;
                                    ...
                                    if self.blockers().get_bit(target).data == 0 {
                                        ...
                                        if !one_move_still_in_check && !is_diagonal_pinned{
                                        ...
                                        if !two_move_still_in_check && !is_diagonal_pinned{
                                    ...
                                    if !is_lateral_pinned {
                                        ...
                                        while attacks.data != 0u64 {
                                            if let Some(target) = attacks.lsb_index() {
                                                ...
                                                // check if attack is on the appropriate diagonal
                                                let mut can_attack_target = true;
                                                if is_diagonal_pinned {
                                                    let king_pos = match self.piece_bbs[0].lsb_index() {
                                                        Some(x) => x,
                                                        None => panic!("generate_moves error: king not found!"),
                                                    };
                                                    let new_blocker = BB{data: blockers.data & !(1u64 << source)};
                                                    let pin_ray = get_bishop_attack(king_pos, new_blocker);
                                                    if pin_ray.data & (1u64 << target) == 0 {
                                    ...
                                    // pawn enpassant
                                    if self.enpassant_bb.data != 0u64 && !self.piece_is_pinned(source) {
                                    ...
                                    while attacks.data != 0 {
                                        if let Some(target) = attacks.lsb_index() {
                                            ...
                                            if is_pinned {
                                                ...
                                                let mut pin_mask = BB{data};
                                                // enemy queen
                                                let mut enemy_queens = self.piece_bbs[1];
                                                while enemy_queens.data != 0 {
                                                    let i = match enemy_queens.lsb_index() {
                                                        Some(x) => x,
                                                        None => unreachable!(),
                                                ...
                                                // enemy bishop
                                                let mut enemy_bishops = self.piece_bbs[3];
                                                while enemy_bishops.data != 0 {
                                                    let i = match enemy_bishops.lsb_index() {
                                                        Some(x) => x,
                                                        None => unreachable!(),
                                                ...
                                                // enemy rook
                                                let mut enemy_rooks = self.piece_bbs[4];
                                                while enemy_rooks.data != 0 {
                                                    let i = match enemy_rooks.lsb_index() {
                                                        Some(x) => x,
                                                        None => unreachable!(),
                            ...
                            while bitboard.data != 0 {
                                if let Some(source) = bitboard.lsb_index() {
                                    ...
                                    if self.enpassant_bb.data != 0u64 {
                                        let data = W_PAWN_ATTACKS[source].data & self.enpassant_bb.data; 
                                        let mut attacks = BB {data};
                                        while attacks.data != 0u64 && !self.piece_is_pinned(source) {
                                            if let Some(target) = attacks.lsb_index() {
                                                // if king is in check, only consider blocking/capturing moves
                                                if self.check_bb.data != 0 {
                                                    // make sure king is no longer in check if move happens //note:probably expensive!!!
